...
# --- ML Model Loading & Configuration ---
MODEL_PATH = os.path.join(os.path.abspath(os.path.dirname(__file__)), 'model', 'best_model.keras')
model = None
try:
    if os.path.exists(MODEL_PATH):
        model = tf.keras.models.load_model(MODEL_PATH, compile=False)
        app.logger.info(f"Model successfully loaded from {MODEL_PATH}")
    else:
        app.logger.error(f"Model file not found at {MODEL_PATH}. Classification will use dummy logic.")
except Exception as e:
    app.logger.error(f"Failed to load model: {e}. Classification will use dummy logic.")
...
def token_required(f):
    ...
    def decorated(*args, **kwargs):
        ...
        except jwt.ExpiredSignatureError: return jsonify({'message': 'Token sudah kedaluwarsa!', 'error_code': 'TOKEN_EXPIRED'}), 401
        except jwt.InvalidTokenError: return jsonify({'message': 'Token tidak valid!', 'error_code': 'TOKEN_INVALID'}), 401
        except Exception as e:
...
# --- ML Prediction Helper Function ---
def predict_image(image_path):
    if model is None:
        app.logger.warning("Model tidak dimuat, menggunakan prediksi dummy.")
        main_category = random.choice(["ORGANIK", "ANORGANIK", "RESIDU"])
        if main_category == "ORGANIK":
            specific_category = "biological"
        elif main_category == "ANORGANIK":
            specific_category = random.choice(['cardboard', 'glass', 'metal', 'paper', 'plastic'])
        else:
            specific_category = random.choice(['battery', 'clothes', 'shoes', 'trash'])
        simulated_accuracy = random.uniform(0.50, 0.98)
        suggestions = [HANDLING_SUGGESTIONS.get(specific_category, "Saran tidak tersedia.")]
        return main_category, specific_category, simulated_accuracy, suggestions, None
    ...
    try:
        ...
        pred_array = predictions[0] if isinstance(predictions, list) else predictions
        predicted_class_index = np.argmax(pred_array[0])
        predicted_label_specific = CLASS_LABELS[predicted_class_index]
        raw_accuracy = float(pred_array[0][predicted_class_index])
        simulated_accuracy = 0.50 + (raw_accuracy * (0.98 - 0.50))
        ...
        return main_category, predicted_label_specific, simulated_accuracy, suggestions, None
    except Exception as e:
        app.logger.error(f"Error saat prediksi gambar {image_path}: {e}")
        return None, None, 0.0, None, f"Terjadi kesalahan saat memproses gambar: {e}"
...
def klasifikasi_sampah_authenticated(current_user):
    ...
    response_data = {
        "kategori": main_category, "kategori_spesifik": specific_category, "akurasi": accuracy,
        "saran_penanganan": suggestions, "timestamp": datetime.datetime.utcnow().isoformat(),
        "nama_file_asli": original_filename, "image_url_preview": image_access_url,
    }
    return jsonify(response_data), 200

# === PERBAIKAN: Menambahkan endpoint baru untuk pengguna tamu ===
...
def klasifikasi_sampah_guest():
    ...
    try:
        ...
        # Menghapus file sementara setelah prediksi selesai
