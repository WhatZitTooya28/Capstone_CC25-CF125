# -*- coding: utf-8 -*-
"""Membangun Model CNN.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1Jov5vBke4zWkTLEw5vZE42q_TJEov5Uh
        ...
        for file in files:
            ...
            try:
                with Image.open(file_path) as img:
                    img.verify()  # cek rusak atau tidak
                # buka lagi untuk pastikan benar-benar bisa dibuka
                with Image.open(file_path) as img:
                    img.load()
            except Exception:
                corrupted.append(file_path)
                os.remove(file_path)
                print(f"Dihapus karena corrupt: {file_path}")
    print(f"\nTotal file corrupt yang dihapus: {len(corrupted)}")

# Panggil fungsi ini pada direktori data kamu
remove_corrupted_images("/content/balanced_dataset")
...
# Jika output adalah dict, ambil array prediksi dari key yang benar, misal 'output_0'
output_key = list(predictions.keys())[0]  # ambil key pertama
pred_array = predictions[output_key]

# Menentukan label untuk setiap kelas
class_labels = ['battery', 'biological', 'cardboard', 'clothes', 'glass', 'metal', 'paper', 'plastic', 'shoes', 'trash']  # Ganti sesuai kelas kamu
...
plt.title(f"Prediksi: {predicted_label}\nProbabilitas: {pred_array[0][predicted_class[0]]:.4f}")
plt.show()